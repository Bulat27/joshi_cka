...
def prepare_coarsened_batches(batch, opts):
        coarsened_batches = {}

        for coarse_ratio in opts.coarse_ratios:
...
def eval_dataset(dataset_path, decode_strategy, width, softmax_temp, opts):
    ...
    dataset = model.problem.make_dataset(
        ...
        neighbors=model_args['neighbors'], knn_strat=model_args['knn_strat'], supervised=True, batching_mode='train', coarse_ratios=opts.coarse_ratios
    )

    pyg_dataset = PyGTSPDataset(root='data/tsp', dataset=dataset)
    # Reusing edge_index without taking up too much memory? Can I do it somehow?
    dataset = get_dataset_with_coarsened_edgelist(pyg_dataset, 'sgc', opts.coarse_ratios, dataset)
    
    cka = _eval_dataset(model, dataset, decode_strategy, width, softmax_temp, opts, device)
...
def _eval_dataset(model,dataset, decode_strategy, width, softmax_temp, opts, device):
    ...
    for batch in tqdm(dataloader, disable=opts.no_progress_bar, ascii=True):
        ...
        new_batches = prepare_coarsened_batches(batch, opts)
        ...
        for coarse_ratio, new_batch in new_batches.items():
            ...
            with torch.no_grad():
                ...
                node_embeddings2 = model.get_node_embeddings(curr_nodes, curr_graph)
